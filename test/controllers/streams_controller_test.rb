...
class TurboClone::StreamsControllerTest < ActionDispatch::IntegrationTest 
  ...
  test "create with respond to" do 
    ...
    #connected to turboclone folder file test assertions to use only one
  ...
  test "update with respond to" do 
    ...
    patch article_path(article), params: { article: { content: "something change"} }
    assert_redirected_to articles_path

    patch article_path(article), params: { article: { content: "something change different"} }, as: :turbo_stream
